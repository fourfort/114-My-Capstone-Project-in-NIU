...
class liteConv_maxpool(nn.Module):
...
class liteShuffleNetV2(nn.Module):
    def __init__(self, inp, oup, stride):  # ch_in, ch_out, stride
        ...
        self.branch2 = nn.Sequential(  # right
            ...
            # nn.Conv2d(branch_features, branch_features, kernel_size=1, stride=1, padding=0, bias=False), #liteconv改進
            # nn.BatchNorm2d(branch_features),
            # nn.ReLU(inplace=True),
        )
    ...
    def forward(self, x): 
        if self.stride == 1:
            ...
            out = torch.cat((x1, self.branch2(x2)), dim=1) # for stride =1 （普通模塊） 
        else:
            out = torch.cat((self.branch1(x), self.branch2(x)), dim=1) # for stride =2 (降採樣) 
